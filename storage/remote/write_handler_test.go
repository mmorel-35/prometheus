...
func TestRemoteWriteHandlerHeadersHandling_V1Message(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			req, err := http.NewRequestWithContext(context.Background(), "", "", bytes.NewReader(payload))
			require.NoError(t, err)
	...
	for _, tc := range testCases {
		b.Run(tc.name, func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				req, err := http.NewRequestWithContext(context.Background(), "", "", bytes.NewReader(buf))
				require.NoError(b, err)
				b.StartTimer()
...
func BenchmarkRemoteWriteOOOSamples(b *testing.B) {
	...
	b.ResetTimer()
	for i := range 100 {
		req, err = http.NewRequestWithContext(context.Background(), "", "", bytes.NewReader(bufRequests[i]))
		require.NoError(b, err)
