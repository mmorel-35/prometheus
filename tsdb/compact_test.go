...
func TestCancelCompactions(t *testing.T) {
	...
	var timeCompactionUninterrupted time.Duration
	{
		...
		require.Zerof(t, prom_testutil.ToFloat64(db.compactor.(*LeveledCompactor).metrics.Ran), "initial compaction counter mismatch")
...
func TestDeleteCompactionBlockAfterFailedReload(t *testing.T) {
	...
	for title, bootStrap := range tests {
		t.Run(title, func(t *testing.T) {
			...
			require.Zerof(t, prom_testutil.ToFloat64(db.metrics.reloadsFailed), "initial 'failed db reloadBlocks' count metrics mismatch")
			require.Zerof(t, prom_testutil.ToFloat64(db.compactor.(*LeveledCompactor).metrics.Ran), "initial `compactions` count metric mismatch")
			require.Zerof(t, prom_testutil.ToFloat64(db.metrics.compactionsFailed), "initial `compactions failed` count metric mismatch")
