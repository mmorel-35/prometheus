...
func TestGetSeries(t *testing.T) {
	...
	request := func(method string, matchers ...string) (*http.Request, error) {
		...
		r, err := http.NewRequestWithContext(context.Background(), method, u.String(), nil)
...
"help":"Number of OS threads that were created."}]}`,
	...
	request := func(m string, q url.Values) (*http.Request, error) {
		if m == http.MethodPost {
			r, err := http.NewRequestWithContext(context.Background(), m, "http://example.com", strings.NewReader(q.Encode()))
			r.Header.Set("Content-Type", "application/x-www-form-urlencoded")
			r.RemoteAddr = "127.0.0.1:20201"
			return r, err
		}
		r, err := http.NewRequestWithContext(context.Background(), m, fmt.Sprintf("http://example.com?%s", q.Encode()), nil)
		r.RemoteAddr = "127.0.0.1:20201"
		return r, err
	}
...
func TestAdminEndpoints(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run("", func(t *testing.T) {
			...
			req, err := http.NewRequestWithContext(context.Background(), tc.method, fmt.Sprintf("?%s", tc.values.Encode()), nil)
			require.NoError(t, err)
...
func TestParseTimeParam(t *testing.T) {
	...
	for _, test := range tests {
		req, err := http.NewRequestWithContext(context.Background(), http.MethodGet, "localhost:42/foo?"+test.paramName+"="+test.paramValue, nil)
		require.NoError(t, err)
...
func TestOptionsMethod(t *testing.T) {
	...
	req, err := http.NewRequestWithContext(context.Background(), http.MethodOptions, s.URL+"/any_path", nil)
...
func BenchmarkRespond(b *testing.B) {
	...
	for _, c := range cases {
		b.Run(c.name, func(b *testing.B) {
			b.ReportAllocs()
			request, err := http.NewRequestWithContext(context.Background(), http.MethodGet, "/does-not-matter", nil)
...
func TestQueryTimeout(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ctx := context.Background()
			req, err := http.NewRequestWithContext(context.Background(), tc.method, fmt.Sprintf("http://example.com?%s", query.Encode()), nil)
			require.NoError(t, err)
